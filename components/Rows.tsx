  ...
  return (
    <div className='relative'>
          <Swiper 
              ...
              navigation={{
                prevEl: '.deal_button_prev',
                nextEl: '.deal_button_next',
            }}
              pagination={false}
              autoplay={false}
              observer={true}
          ...
          >
            ...
            {data.length > 0 && data.map((item: StayInfo, index: number) => {
              if(index <= 10) {
                return (
                  <SwiperSlide key={data[index].contentid}>
                    <div className='cursor-pointer' onClick={handleClick} data-id={data[index].contentid}>
                      ...
                      <p className='mt-[15px] font-[600] text-[16px] text-right'>{data[index].price}원<span className='font-[300]'>~</span></p>
                    </div>
                  </SwiperSlide>
                  )
              }
          ...
          <button className='deal_button_prev bg-gray-800 p-2 rounded absolute top-[calc(50%-9px)] left-0 transform -translate-y-1/2 z-10 cursor-pointer'><FaAngleLeft  size={20} color='#fff'/></button>
          <button className='deal_button_next bg-gray-800 p-2 rounded absolute top-[calc(50%-9px)] right-0 transform -translate-y-1/2 z-10 cursor-pointer'><FaAngleRight size={20} color='#fff'/></button>
        </div>
  )
}
